...
class RaindropsGame:
    ...
    def __init__(self):
        ...
        self.omgraindropwow = pygame.sprite.Group()
        self._create_drops()
    ...
    def _create_drop(self, drop_number, row_number):
        ...
        drop = Raindrop(self)
        drop_width, drop_height = drop.rect.size
        drop.rect.x = drop.y + 2 * drop_width * drop_number
        drop.y = 2 * drop.rect.height * row_number
        drop.rect.y = drop.y
        self.omgraindropwow.add(drop)

    def _update_raindrops(self):
        """Update drops position and look for drops that have disapeared"""
        self.omgraindropwow.update()

        # Assume we won't make new drops
        make_new_drops = False
        for drop in self.omgraindropwow.copy():
            if drop.check_disappeared():
                # Remove this drop and we'll need to make new drops
                self.omgraindropwow.remove(drop)
                make_new_drops = True
    ...
    def _update_screen(self):
        ...
        self.omgraindropwow.draw(self.screen)
